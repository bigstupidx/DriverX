...
public class RCCCarControllerV2 : MonoBehaviour {
	...
	private float deltaWheelRotation = 3;
...
//		_wheelParticles[3].transform.localPosition = Vector3.zero;
	...
	public void Controlling (){
        ...
        motorInput = Mathf.Clamp(motorInput+Time.deltaTime, 0, 1);
        ...
        switch(directionWheel)
        {
            case InputController.Direction.LEFT:
                wheelRotation = Mathf.Clamp(wheelRotation - deltaWheelRotation * Time.deltaTime,-1,1);
                break;
            ...
            case InputController.Direction.NONE:
                if(wheelRotation < 0)
                    wheelRotation = Mathf.Clamp(wheelRotation + deltaWheelRotation * Time.deltaTime, -1, 0);
                else if(wheelRotation >0)
                    wheelRotation = Mathf.Clamp(wheelRotation - deltaWheelRotation * Time.deltaTime, 0, 1);
                break;
        }

        steerInput = Mathf.Lerp (steerInput, wheelRotation, Time.deltaTime * 10);
